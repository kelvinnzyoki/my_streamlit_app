...
function progressFromUsageRecords(records: Array<{ action?: string }> = []): Record<string, number[]> {
  const progress: Record<string, number[]> = {};

  records.forEach((record) => {
    const action = String(record?.action || '');
    const match = action.match(/^WORKOUT_LOGGED:(\d+):(\d+)$/);
    if (!match) return;
    ...
    const existing = new Set(progress[dayIndex] || []);
    existing.add(exerciseIndex);
    progress[dayIndex] = Array.from(existing).sort((a, b) => a - b);
  });
...
export default function ProgramDetailPage() {
  ...
  useEffect(() => {
    ...
    setExerciseProgress(progressFromUsageRecords(enrollment.usageRecords || enrollment.usage_records || []));
  }, [enrollment]);

  useEffect(() => {
    if (searchParams.get('logged') === '1') {
      setMessage('Workout logged. Your server progress has been updated.');
      const cleanUrl = program?.id
        ? `/programs/${encodeURIComponent(program.id)}${enrollment?.id ? `?enrollmentId=${encodeURIComponent(enrollment.id)}` : ''}`
        : null;
      if (cleanUrl) router.replace(cleanUrl);
    }
  }, [searchParams, program?.id, enrollment?.id, router]);
  ...
  async function enroll(restart = false) {
    ...
    try {
      ...
      setExerciseProgress(progressFromUsageRecords(mergedEnrollment.usageRecords || mergedEnrollment.usage_records || []));
  ...
  function sessionHref(exercise: ProgramExercise) {
    ...
    const nextDay = programDays[Math.min(exercise.globalDayIndex + 1, Math.max(programDays.length - 1, 0))];
    const returnUrl = `/programs/${encodeURIComponent(program.id)}?enrollmentId=${encodeURIComponent(enrollmentId)}&logged=1`;
    ...
    const params = new URLSearchParams({
      ...
      exerciseIndex: String(exercise.exerciseIndex),
      dayExerciseCount: String(currentDay?.exercises.length || 1),
      currentWeek: String(currentDay?.weekNumber || 1),
      currentDay: String(currentDay?.dayNumber || 1),
      nextWeek: String(nextDay?.weekNumber || currentDay?.weekNumber || 1),
      nextDay: String(nextDay?.dayNumber || currentDay?.dayNumber || 1),
      returnUrl,
    });
