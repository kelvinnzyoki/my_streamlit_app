...
export default function ProgramCard({ program }: { program: Program }) {
  ...
  return (
    <article className="premium-card content-card program-card">
      <img className="card-img" src={imageUrl(program.image)} alt={program.title} loading="lazy" />
      <div className="card-body">
        ...
        <div className="pill-row">
          ...
          <span className="pill">{program.focus || 'Full body'}</span>
        </div>
        <Link className="primary-btn card-btn" href={`/programs/${id}`}>
          View Program
        </Link>
      </div>
    </article>
